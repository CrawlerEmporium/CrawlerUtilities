...
from crawler_utilities.utils.globals import HELP
...
class Help(commands.HelpCommand):
    ...
    async def send_bot_help(self, mapping):
        helpCommands = await HELP['help'].find({"bots": self.identifier, "disabled": None}).to_list(length=None)
    ...
    async def getCommandHelp(self, command):
        helpCommand = await HELP['help'].find_one({"bots": self.identifier, "command": command})
    ...
    async def list_embed(self, helpCommands, ctx):
        ...
        for i in range(0, len(helpCommands), 10):
            ...
            embed = discord.Embed(title=f"{self.identifier.capitalize()}Crawler Commands",
                                  description=f"A list of all the commands that the {self.identifier.capitalize()}Crawler bot has to offer. Find out more by typing in ``{prefix}help [command]``")
