...
def getNumberedButtons(numbers):
    ...
    for x in pages[0]:
        view.add_item(Button(style=1, custom_id=f"choice {x}", label=x, row=0))
    if len(pages) > 1:
        for x in pages[1]:
            view.add_item(Button(style=1, custom_id=f"choice {x}", label=x, row=1))
        return view, 1
    return view, 0
...
class EmbedPaginator(Dialog):
    ...
    def getPaginationButtons(self, view, row, first=False, second=False, fourth=False, fifth=False):
        view.add_item(Button(style=1, custom_id=self.control_labels[0], emoji=self.control_emojis[0], disabled=first, row=row))
        view.add_item(Button(style=1, custom_id=self.control_labels[1], emoji=self.control_emojis[1], disabled=second, row=row))
        view.add_item(Button(style=4, custom_id=self.control_labels[4], emoji=self.control_emojis[4], row=row))
        view.add_item(Button(style=1, custom_id=self.control_labels[2], emoji=self.control_emojis[2], disabled=fourth, row=row))
        view.add_item(Button(style=1, custom_id=self.control_labels[3], emoji=self.control_emojis[3], disabled=fifth, row=row))
        return view
    ...
    async def run(self, users: List[discord.User], channel: discord.TextChannel = None, valid=None):
        ...
        if len(valid) > 0:
            ...
            if len(self.pages) == 1:
                view = discord.ui.View()
                view.add_item(Button(style=4, custom_id=self.control_labels[4], emoji=self.control_emojis[4], row=0))
            else:
                view = discord.ui.View()
